...
setwd("/home/carlsonlab/KMReef")
#create vector that denotes path
path <- "/home/carlsonlab/KMReef"
...
taxa <- assignTaxonomy(seqtab.nochim, "/home/carlsonlab/KMReef/silva_nr99_v138.1_wSpecies_train_set.fa.gz")
...
write.table(cbind(t(seqtab.nochim) , taxa), "KMReef_seqtab-nochimtaxa.txt", sep="\t", row.names=TRUE, col.names=NA, quote=FALSE)
#write our taxa file
write.table(taxa,"KMReef_taxa.txt", sep="\t", row.names=TRUE, col.names=NA, quote=FALSE)
