...
#load data
ASV_nochl.mit.euk <- read.csv("KMReef_seqtab-nochimtaxa_NoChlMitEuk.csv", header=TRUE,sep=",",row.names=1, na.strings = "NOPE")
TAX_nochl.mit.euk <- read.csv("KMReef_taxa_NoChlMitEuk.csv", header=TRUE,sep=",", row.names = 1, na.strings = "NOPE")
meta <- read.csv("KMReef_metadata.csv", header=TRUE,sep=",",row.names=1)

#generate rarefaction curve
ASV <- ASV_nochl.mit.euk[,1:(ncol(ASV_nochl.mit.euk)-7)]
rarecurve(tASV, step = 1000, col = "blue", label=F, xlim=c(0,20000))

#rarefy samples to 3000
...
set.seed(8800)
rar <- rarefy_even_depth(phy, sample.size = 3000)
...
#remove singletons (1/3000*100=0.033%)
single <- rowSums(ASV_rar_prop[,1:ncol(ASV_rar_prop)]) > 0.033
...
write.csv(taxa_NoSingle, file="KMReef_taxa_rar3000_NoSingle.csv")
write.csv(ASV_NoSingle, file="KMReef_seqtab-nochimtaxa_rar3000_NoSingle.csv")
write.csv(meta_rar,file="KMReef__meta_rar3000.csv")
